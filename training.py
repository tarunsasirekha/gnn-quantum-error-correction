...
class LogicalErrorLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        ...
        """
        # Base BCE loss
        base = self.base_loss(predictions.squeeze(), targets.squeeze().float())

        # Logical error penalty — handles batched graphs by reshaping
        pred_binary = (torch.sigmoid(predictions.squeeze()) > 0.5).float()
        n = self.logical_ops.shape[1]
        if pred_binary.numel() % n == 0:
            batch_size = pred_binary.numel() // n
            pred_reshaped = pred_binary.view(batch_size, n)          # [B, n]
            logical_ops = self.logical_ops.to(predictions.device)    # [k, n]
            logical_overlap = (pred_reshaped @ logical_ops.t()) % 2  # [B, k]
            logical_error_rate = logical_overlap.float().mean()
        else:
            logical_error_rate = torch.tensor(0.0, device=predictions.device)
...
class Trainer:
    ...
    def train_epoch(self, data_list: List) -> Dict[str, float]:
        ...
        for batch in tqdm(loader, desc="Training"):
            ...
            loss_output = self.criterion(predictions.squeeze(), batch.y)
            loss = loss_output[0] if isinstance(loss_output, tuple) else loss_output

            loss.backward()
            # Fix 4: gradient clipping prevents exploding gradients on large graphs
    ...
    def validate(self, data_list: List) -> Dict[str, float]:
        """Validate on validation set using mini-batches"""
        from torch_geometric.loader import DataLoader
        self.model.eval()
    ...
    def train(self,
        ...
        Returns:
            History dictionary
        """
        # Fix 5: track best model by F1 (better for imbalanced error data)
        best_val_f1 = -1.0
