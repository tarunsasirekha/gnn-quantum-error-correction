...
class AttentionGNNLayer(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_index_i):
        ...
        Args:
            ...
            edge_index_i: Target node indices [num_edges] — used for per-node softmax
        """
        ...
        # Attention scores — normalize per target node (not over all edges)
        alpha = (query * key).sum(dim=-1) / math.sqrt(self.out_channels)
        alpha = pyg_softmax(alpha, edge_index_i)
...
class GNNDecoder(nn.Module):
    ...
    def __init__(self, 
        ...
        # Project check node context into qubit space
        self.check_proj = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, data):
        ...
        # Extract qubit and check features using mask (works with batched graphs)
        qubit_mask = data.qubit_mask if hasattr(data, 'qubit_mask') else torch.arange(n_qubits, device=x.device)
        qubit_features = x[qubit_mask]
        check_features = x[~qubit_mask] if hasattr(data, 'qubit_mask') else x[n_qubits:]

        # Incorporate check node context into qubit features
        if hasattr(data, 'batch') and data.batch is not None:
            check_batch = data.batch[~qubit_mask]
            qubit_batch = data.batch[qubit_mask]
            num_graphs = int(data.batch.max().item()) + 1
            check_sum = torch.zeros(num_graphs, check_features.size(-1), device=x.device)
            check_sum.scatter_add_(0, check_batch.unsqueeze(1).expand_as(check_features), check_features)
            check_count = torch.bincount(check_batch, minlength=num_graphs).float().unsqueeze(1).clamp(min=1)
            check_context = (check_sum / check_count)[qubit_batch]
        else:
            check_context = check_features.mean(0, keepdim=True).expand(qubit_features.size(0), -1)

        qubit_features = qubit_features + self.check_proj(check_context)
...
class ResidualGNNDecoder(nn.Module):
    ...
    def forward(self, data):
        ...
        # Extract qubit/check features and add check context
        qubit_mask = data.qubit_mask if hasattr(data, 'qubit_mask') else torch.arange(n_qubits, device=x.device)
        qubit_features = x[qubit_mask]
        check_features = x[~qubit_mask] if hasattr(data, 'qubit_mask') else x[n_qubits:]
