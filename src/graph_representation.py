...
class TannerGraphBuilder:
    ...
    def _create_qubit_features(self, add_features: bool = True) -> torch.Tensor:
        ...
        if add_features:
            # Feature: [is_qubit, degree, 0 (placeholder for error prediction)]
            degrees = self.H.sum(axis=0)  # Number of checks each qubit participates in
            
            features = torch.zeros((self.n_qubits, 3), dtype=torch.float32)
            features[:, 0] = 1.0  # is_qubit indicator
            features[:, 1] = torch.tensor(degrees, dtype=torch.float32)  # degree
            # features[:, 2] stays 0 (will be updated during message passing)
        else:
            # Minimal features
    ...
    def _create_check_features(self, syndrome: np.ndarray, add_features: bool = True) -> torch.Tensor:
        ...
        if add_features:
            # Feature: [is_check, syndrome_value, degree]
            degrees = self.H.sum(axis=1)  # Number of qubits each check involves
            ...
            features[:, 2] = torch.tensor(degrees, dtype=torch.float32)
        else:
