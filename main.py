...
from training import Trainer, DecoderLoss, LogicalErrorLoss
...
def train_gnn_decoder(train_data, val_data, args, logical_ops=None):
    """Train GNN decoder"""
    print("\n" + "="*80)
    ...
    # Warm-start from a previous checkpoint if provided
    if args.load_model:
    ...
    if logical_ops is not None:
        criterion = LogicalErrorLoss(logical_operators=logical_ops, base_loss_weight=1.0)
        print("  Loss: LogicalErrorLoss (base_loss_weight=1.0)")
    else:
        criterion = DecoderLoss(loss_type='focal')
        print("  Loss: Focal (no logical operators available)")
...
def main():
    ...
    parser.add_argument('--hidden-dim', type=int, default=128,
                      help='Hidden dimension (increased from 64 for better performance)')
    parser.add_argument('--num-layers', type=int, default=6,
                      help='Number of GNN layers (increased from 5)')
    parser.add_argument('--dropout', type=float, default=0.05,
                      help='Dropout rate (reduced from 0.1)')
    
    # Training parameters
    parser.add_argument('--epochs', type=int, default=30,
                      help='Number of training epochs (increased from 20)')
    ...
    # Get logical operators for loss function
    logical_ops_X = None
    if hasattr(code, 'get_logical_operators'):
        logical_ops_X, _ = code.get_logical_operators()

    # Train model
    model, history, training_time = train_gnn_decoder(train_data, val_data, args, logical_ops=logical_ops_X)
    ...
    print(f"\nFinal Results:")
    print(f"  {'Decoder':<25} {'Success Rate':>14} {'Avg Decode Time':>17}")
    print(f"  {'-'*58}")
    print(f"  {gnn_r.name:<25} {gnn_r.success_rate*100:>13.2f}%  {gnn_r.avg_decode_time*1000:>13.3f} ms")
    print(f"  {bp_r.name:<25} {bp_r.success_rate*100:>13.2f}%  {bp_r.avg_decode_time*1000:>13.3f} ms")
    print(f"  {'-'*58}")

    success_diff = gnn_r.success_rate - bp_r.success_rate
    speedup = bp_r.avg_decode_time / gnn_r.avg_decode_time if gnn_r.avg_decode_time > 0 else float('inf')

    if success_diff >= 0:
        print(f"\n  GNN outperforms BP by {success_diff*100:.2f} percentage points in success rate")
        print(f"  GNN is {speedup:.1f}x {'faster' if speedup >= 1 else 'slower'} than BP per decode")
    else:
        print(f"\n⚠️  WARNING: GNN underperformed BP by {abs(success_diff)*100:.2f} percentage points")
        print(f"  This suggests: insufficient training, bad hyperparameters, or dataset issues")
        print(f"  Try: more epochs, more training data, or different learning rate")
        print(f"  Decode time — GNN: {gnn_r.avg_decode_time*1000:.3f} ms  |  BP: {bp_r.avg_decode_time*1000:.3f} ms")
